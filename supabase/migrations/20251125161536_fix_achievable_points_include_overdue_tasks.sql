/*
  # Include Overdue Tasks in Today's Achievable Points

  ## Problem
  - User has overdue tasks from yesterday
  - User completes them today and gets points
  - But achievable for today doesn't include them
  - Result: Achieved >> Achievable (looks broken)

  ## Philosophy Decision
  If a task is OPEN (not completed), the user can still complete it TODAY
  and earn points. Therefore it SHOULD be in today's achievable!

  ## Solution
  - Achievable = ALL open tasks assigned to user (including overdue)
  - This matches reality: user CAN earn these points today
  - Percentage then shows realistic progress

  ## Note
  - Tasks due TODAY get priority (should be done first)
  - But overdue tasks are still achievable opportunities
*/
...
BEGIN
  ...
  -- User can complete ANY open task today and get points
...
'Calculates achievable points including ALL open tasks (even overdue). User can complete them today!';
