/*
  # Fix process_check_in Date Type

  ## Problem:
  - check_in_date column is type 'date'
  - Function tries to insert 'text'
  
  ## Solution:
  - Cast v_today_date to date type
*/
...
DECLARE
  ...
  v_today_date date;  -- Changed from text to date
...
BEGIN
  v_check_in_time := now();
  v_today_date := DATE(v_check_in_time AT TIME ZONE 'Asia/Phnom_Penh');  -- Now returns date type
  ...
  VALUES (
    p_user_id, 
    v_check_in_time, 
    v_today_date,  -- Now correct date type
    v_shift_type, 
    v_is_late, 
    v_minutes_late, 
    v_points,
  ...
  -- Send push notification to all admins (if function exists)
