/*
  # Fix approve_task_with_quality - Remove task_id column reference
  
  1. Problem
    - Function tries to insert task_id into points_history
    - But points_history table doesn't have task_id column
    - This causes 400 error when admin approves tasks
  
  2. Solution
    - Remove task_id from INSERT statements
...
BEGIN
  ...
  -- Award points to primary staff (REMOVED task_id column)
  ...
  -- Award 50% points to helper if exists (REMOVED task_id column)
  IF v_helper_id IS NOT NULL AND v_helper_id != v_staff_id AND v_helper_points > 0 THEN
    INSERT INTO points_history (user_id, points_change, category, reason, created_by)
    VALUES (v_helper_id, v_helper_points, 'task_completed', v_task.title || ' (Helfer)', p_admin_id);
...
'Approves a task with a quality rating. Awards points based on base value + quality bonus + deadline bonus. 
If task has a helper, awards 50% of total points to helper.';
