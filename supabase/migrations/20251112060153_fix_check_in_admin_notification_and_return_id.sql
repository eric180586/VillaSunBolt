/*
  # Fix Check-In System - Admin Notification & Return ID

  1. Problem
    - process_check_in sends notification only to user, NOT to admin
    - process_check_in doesn't return check_in_id
    - Admin has no visibility when staff checks in

  2. Solution
    - Return check_in_id in response
    - Send notification to ALL admins when staff checks in
    - Include user name, shift type, time, and late status in notification

  3. Changes
    - Update process_check_in to return check_in_id
