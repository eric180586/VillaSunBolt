/*
  # Fix approve_task_with_items Category Value
  
  ## Problem
  Function uses 'task_approval' category but constraint only allows:
  - task_completed
  - bonus
  - deduction
  - achievement
  ...
  ## Solution
  Change category to 'task_completed' which is the correct category
*/
...
BEGIN
  ...
  IF p_approved THEN
    ...
    -- Award points to primary user (FIXED: use 'task_completed' category)
    ...
    -- Award points to helper if exists (FIXED: use 'task_completed' category)
    IF v_task.secondary_assigned_to IS NOT NULL THEN
      INSERT INTO points_history (user_id, points_change, reason, category, created_by, created_at)
      VALUES (
