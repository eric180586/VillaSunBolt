/*
  # Skip daily_point_goals update for custom points to avoid overflow

  When admin uses custom points, skip the daily_point_goals calculation
  to prevent numeric overflow errors.
*/
...
BEGIN
  ...
  -- Only update daily goals for normal check-ins, not custom points
  ...
  ELSE
    INSERT INTO notifications (user_id, title, message, type)
    VALUES (v_check_in.user_id, 'Check-in Approved', 'Check-in confirmed. ' || CASE WHEN v_points_awarded = 0 THEN 'On time - no penalty.' ELSE v_points_awarded || ' points awarded!' END, 'success');
  END IF;
