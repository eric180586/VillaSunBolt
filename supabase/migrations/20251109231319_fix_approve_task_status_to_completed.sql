/*
  # Fix approve_task_with_quality - Use 'completed' status

  1. Changes
    - Change status from 'approved' to 'completed' when approving tasks
    - 'approved' is not a valid status in the tasks_status_check constraint
    - Valid statuses: pending, in_progress, pending_review, completed, cancelled, archived

  2. Notes
    - This fixes the constraint violation error when approving tasks
    - Tasks should be set to 'completed' when admin approves them
*/
...
BEGIN
  ...
  -- Update task status to 'completed' (not 'approved')
  UPDATE tasks 
  SET 
    status = 'completed',
    admin_reviewed = true,
