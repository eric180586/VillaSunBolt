/*
  # Fix approve_task_with_quality - Add category column back

  1. Changes
    - Add category column back to points_history inserts
    - The category column DOES exist in points_history table
    - Use task category for proper point categorization

  2. Notes
    - points_history structure: id, user_id, points_change, reason, category, created_by, created_at
    - category has default value 'other' but we should use task category
*/
...
BEGIN
  ...
  -- Award points to primary staff with category
  IF v_staff_id IS NOT NULL THEN
    ...
    VALUES (
      ...
      COALESCE(v_task.category, 'task_completed'),
      p_admin_id
    );
  END IF;
