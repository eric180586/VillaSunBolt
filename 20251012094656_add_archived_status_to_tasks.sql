/*
  ...
  1. Changes
    - Update task status enum to include 'archived'
    - Modify approve_task_with_points to set status to 'archived' instead of 'completed'
    - Ensure archived tasks are excluded from most queries

  2. Security
    - Maintains existing RLS policies
    - Only admins can approve and archive tasks
*/

-- Update the status check constraint to include 'archived'
ALTER TABLE tasks DROP CONSTRAINT IF EXISTS tasks_status_check;
ALTER TABLE tasks ADD CONSTRAINT tasks_status_check
  CHECK (status IN ('pending', 'in_progress', 'completed', 'cancelled', 'pending_review', 'archived'));

-- Update the approve_task_with_points function to archive tasks after approval
