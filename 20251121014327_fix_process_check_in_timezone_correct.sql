/*
  # Fix process_check_in timezone conversion
  
  NOW() returns timestamptz which is already in UTC.
  We just need to convert to Asia/Phnom_Penh, not AT TIME ZONE 'UTC' first.
*/
...
BEGIN
  ...
  -- Convert UTC to Cambodia time (NOW() is already timestamptz in UTC)
  ...
  -- Check if already checked in today (Cambodia date)
  SELECT EXISTS(
