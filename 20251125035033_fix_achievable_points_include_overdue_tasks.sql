/*
  # Fix Achievable Points - Include Overdue Tasks

  1. Problem
    - Function only counts tasks due today or created today
    - But OVERDUE tasks (from yesterday or earlier) should also count!
    - This causes "34 of 0" - user has points from old tasks that aren't counted
    
  2. Solution
    - Count ALL open tasks assigned to user (not just today's)
    - These are tasks they can still complete and earn points for
    - Don't filter by date for assigned tasks - if it's open, it's achievable!
*/
...
BEGIN
  ...
  -- 2. Unassigned tasks (only today's tasks, only if has schedule)
  ...
  -- 3. Tasks assigned to this user (ALL open tasks, including overdue!)
  SELECT v_achievable_points + COALESCE(SUM(t.points_value), 0) INTO v_achievable_points
  FROM tasks t
  ...
  -- 4. Helper tasks (ALL open tasks, including overdue!)
  ...
  -- 5. Patrol rounds scheduled for today only
  ...
  -- 6. Checklist instances for today only
