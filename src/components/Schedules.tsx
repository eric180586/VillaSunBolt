...
export function Schedules({ onNavigate, onBack }: SchedulesProps = {}) {
  ...
  const handleShiftClick = async (staffId: string, dayIndex: number, currentShift?: ShiftType) => {
    ...
    const newShifts: DayShift[] = DAYS.map((day, index) => {
      const existingShift = schedule?.shifts.find((s: any) => s.day === day.toLowerCase());
      if (index === dayIndex) {
        return {
          day: day.toLowerCase(),
  ...
  const weekDates = getWeekDates(currentWeekStart);
  const pendingRequests = timeOffRequests.filter((r: any) => r.status === 'pending');
  const isPublished = weekSchedules.some((s: any) => s.is_published);
                ...
                return (
                  <tr key={staff.id} className="border-b border-gray-100 hover:bg-beige-50">
                    ...
                    {DAYS.map((day, dayIndex) => {
                      const dayShift = schedule?.shifts.find((s: any) => s.day === day.toLowerCase());
